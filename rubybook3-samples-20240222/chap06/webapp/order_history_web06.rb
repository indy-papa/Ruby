...
require_relative '../order_history_excel_writer' # <1>
...
class OrderHistoryWebApp
  ...
  def add_excel_trans_proc # <1>
    @server.mount_proc('/excel_trans') do |req, res|
      p req.query
      term = req.query['term']
      file = req.query['file']
      exfile = req.query['exfile'] # <2>
      begin
        app = OrderHistoryExcelWriter.new(file, exfile) # <3>
        app.write_order_history # <4>
        erb = 'excel_trans_succeeded.erb' # <5>
      rescue => e
        errormsg = e
        erb = 'excel_trans_failed.erb' # <6>
      end
      template = ERB.new(File.read(erb))
      res.body << template.result(binding)
    end
